...
static void usage( char * text ) {
	printf( "\n%s - %s\n", text, "Picoblaze Disassembler utility V1.1" ) ;
	printf( "\nUSAGE:\n" ) ;
	printf( "   pBlazDIS [-6] [-x] [-v] <MEM inputfile> <PSM outputfile>\n" ) ;
}
...
static bool writePSM3( const char * strPSMfile ) {
	...
	for ( pc = 0 ; pc < 1024 ; ) {
		...
		case stCODE :
			if ( c != 0 ) {
				...
				case 0x20000 ... 0x20FFF :
					...
					default :
						fprintf( outfile, "\tINST\t0x%.5X\t; %.3X : %.5X\n", c, pc, c ) ;
					}
					break ;
				...
				case 0x3C000 :
					fprintf( outfile, "\tDINT\t \t; %.3X : %.5X\n", pc, c ) ;
					break ;
				case 0x3C001 :
					fprintf( outfile, "\tEINT\t \t; %.3X : %.5X\n", pc, c ) ;
					break ;
				case 0x38000 :
					fprintf( outfile, "\tRETI\tDISABLE\t; %.3X : %.5X\n", pc, c ) ;
					break ;
				case 0x38001 :
					fprintf( outfile, "\tRETI\tENABLE\t; %.3X : %.5X\n", pc, c ) ;
					break ;
		...
		case stDATA :
			if ( c != 0 ) {
				fprintf( outfile, "\t.BYT\t0x%.2X, 0x%.2X\t; %.3X : %.5X\n", c & 0xFF, ( c >> 8 ) & 0xFF, pc, c ) ;
				pc += 1 ;
			} else
				state = stIDLE ;
			break ;
		...
		case stCODE :
			if ( c != 0 ) {
				...
				case 0x16000 ... 0x16FFF :
					fprintf( outfile, "\tSTAR\ts%X, s%X  \t; %.3X : %.5X\n", DestReg( c ), SrcReg( c ), pc, c ) ;
					break ;
				...
				case 0x0D000 ... 0x0DFFF :
					...
					break ;
				case 0x0E000 ... 0x0EFFF :
					fprintf( outfile, "\tTSTC\ts%X, s%X  \t; %.3X : %.5X\n", DestReg( c ), SrcReg( c ), pc, c ) ;
					break ;
				case 0x0F000 ... 0x0FFFF :
					fprintf( outfile, "\tTSTC\ts%X, 0x%.2X\t; %.3X : %.5X\n", DestReg( c ), Constant( c ), pc, c ) ;
					break ;
				...
				case 0x1D000 ... 0x1DFFF :
					...
					break ;
				case 0x1E000 ... 0x1EFFF :
					fprintf( outfile, "\tCMPC\ts%X, s%X  \t; %.3X : %.5X\n", DestReg( c ), SrcReg( c ), pc, c ) ;
					break ;
				case 0x1F000 ... 0x1FFFF :
					fprintf( outfile, "\tCMPC\ts%X, 0x%.2X\t; %.3X : %.5X\n", DestReg( c ), Constant( c ), pc, c ) ;
					break ;

				case 0x14000 ... 0x14FFF :
					if ( c & 0xF0 ) {
						fprintf( outfile, "\tCORE\ts%X   \t; %.3X : %.5X\n", DestReg( c ), pc, c ) ;
					} else
						switch ( c & 0xF ) {
						case 0x2 :
							fprintf( outfile, "\tRL  \ts%X      \t; %.3X : %.5X\n", DestReg( c ), pc, c ) ;
				...
				case 0x32000 ... 0x32FFF :
					fprintf( outfile, "\tJUMP\tZ, 0x%.3X\t; %.3X : %.5X\n", Address12( c ), pc, c ) ;
					break ;
				case 0x36000 ... 0x36FFF :
					fprintf( outfile, "\tJUMP\tNZ, 0x%.3X\t; %.3X : %.5X\n", Address12( c ), pc, c ) ;
					break ;
				case 0x3A000 ... 0x3AFFF :
					fprintf( outfile, "\tJUMP\tC, 0x%.3X\t; %.3X : %.5X\n", Address12( c ), pc, c ) ;
					break ;
				case 0x3E000 ... 0x3EFFF :
					fprintf( outfile, "\tJUMP\tNC, 0x%.3X\t; %.3X : %.5X\n", Address12( c ), pc, c ) ;
					break ;
				case 0x26000 ... 0x26FFF :
					fprintf( outfile, "\tJUMP\ts%X, s%X  \t; %.3X : %.5X\n\n", DestReg( c ), SrcReg( c ), pc, c ) ;
					break ;
				...
				case 0x20000 ... 0x20FFF :
					...
					break ;
				case 0x30000 ... 0x30FFF :
					fprintf( outfile, "\tCALL\tZ, 0x%.3X\t; %.3X : %.5X\n", Address12( c ), pc, c ) ;
					break ;
				case 0x34000 ... 0x34FFF :
					fprintf( outfile, "\tCALL\tNZ, 0x%.3X\t; %.3X : %.5X\n", Address12( c ), pc, c ) ;
					break ;
				case 0x38000 ... 0x38FFF :
					fprintf( outfile, "\tCALL\tC, 0x%.3X\t; %.3X : %.5X\n", Address12( c ), pc, c ) ;
					break ;
				case 0x3C000 ... 0x3CFFF :
					fprintf( outfile, "\tCALL\tNC, 0x%.3X\t; %.3X : %.5X\n", Address12( c ), pc, c ) ;
					break ;
				case 0x24000 ... 0x24FFF :
					fprintf( outfile, "\tCALL\ts%X, s%X  \t; %.3X : %.5X  \n", DestReg( c ), SrcReg( c ), pc, c ) ;
					break ;
				...
				case 0x2D000 ... 0x2DFFF :
					...
					break ;
				case 0x2B000 ... 0x2BFFF :
					fprintf( outfile, "\tOUTK\t0x%.2X, 0x%X\t; %.3X : %.5X\n", ( c >> 4 ) & 0xFF, c & 0xF, pc, c ) ;
					break ;
				...
				case 0x37000 :
					fprintf( outfile, "\tBANK\tA\t; %.3X : %.5X\n", pc, c ) ;
					break ;
				case 0x37001 :
					fprintf( outfile, "\tBANK\tB\t; %.3X : %.5X\n", pc, c ) ;
					break ;
...
int main( int argc, char *argv[] ) {
	...
	opterr = -1 ;
	while ( ( optch = getopt( argc, argv, "6hvx" ) ) != -1 ) {
		switch ( optch ) {
		case '6' :
			bKCPSM6 = true ;
	...
	if ( strrchr( src_filename, '.' ) == NULL )
		strcat( src_filename, bXDL ? ".xdl" : ".mem" ) ;
	if ( bVerbose )
		printf( "! %s file: %s\n", bXDL ? "XDL" : "MEM", src_filename ) ;

	// output filename
	if ( argv[ optind ] == NULL ) {
		strcpy( psm_filename, filename( src_filename ) ) ;
	} else {
	...
	if ( bXDL ? loadXDL( src_filename ) : loadMEM( src_filename ) ) {
		if ( bKCPSM6 )
